...
export const PremiumPricing = () => {
  ...
  return (
    <section id="pricing" className="py-24 relative z-10 border-t border-slate-200/50 dark:border-white/5 bg-white dark:bg-[#020617]">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-20">
          ...
          <p className="text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
            Start for free. Upgrade when you need the power of AI. No hidden fees.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center max-w-5xl mx-auto">
          {plans.map((plan, idx) => (
            ...
            >
              ...
              <p className={`text-sm mb-6 ${plan.popular ? 'text-slate-300' : 'text-slate-500 dark:text-slate-400'}`}>{plan.desc}</p>
              
              <div className="mb-8">
                <span className={`text-5xl font-black tracking-tight ${plan.popular ? 'text-white' : 'text-slate-900 dark:text-white'}`}>{plan.price}</span>
                <span className={`text-sm font-medium ml-2 ${plan.popular ? 'text-slate-300' : 'text-slate-500 dark:text-slate-400'}`}>{plan.period}</span>
              </div>
              
              <ul className="space-y-4 mb-8">
                {plan.features.map((feature, fIdx) => (
                  <li key={fIdx} className="flex items-center gap-3">
                    <Check className={`w-5 h-5 shrink-0 ${plan.popular ? 'text-cyber-blue' : 'text-electric-blue'}`} />
                    <span className={`text-sm font-medium ${plan.popular ? 'text-slate-200' : 'text-slate-700 dark:text-slate-300'}`}>{feature}</span>
                  </li>
                ))}
              </ul>
